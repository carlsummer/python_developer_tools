...
from python_developer_tools.cv.bases.conv.CondConv import CondConv
    ...
    for i, (m, name) in enumerate(zip(model.modules(), model.named_modules())):
        if type(m) is nn.Conv2d and m.in_channels > 4:
            dyReluchannels.append({"name": name, "dyrelu":
                                   CondConv(in_planes=m.in_channels, out_planes=m.out_channels, kernel_size=m.kernel_size, stride=m.stride, padding=m.padding,
                                                bias=m.bias)
                                   })
