...
def convert_relu_to_BlurPool(model):
    ...
    for i, (m, name) in enumerate(zip(model.modules(), model.named_modules())):
        if type(m) is nn.Conv2d and m.in_channels > 4:
            dyReluchannels.append({"name": name, "dyrelu": DY_Conv2d(m.in_channels,m.out_channels,kernel_size=m.kernel_size,stride=m.stride,padding=m.padding,dilation=m.dilation,groups=m.groups,bias=m.bias)})
